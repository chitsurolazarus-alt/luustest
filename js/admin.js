...
function renderTrips() {
    ...
    document.querySelector('#tripsTable tbody').innerHTML = AdminState.trips.map(t => `
        <tr>
            ...
            <td>${formatCurrency(t.total_price)}</td>
            <td>
                <select class="select-status" data-trip-status-for="${t.id}">
                    ${tripStatuses.map(s => `<option value="${s}" ${s === t.status ? 'selected' : ''}>${s}</option>`).join('')}
                </select>
            </td>
            <td class="table-actions">
    ...
    document.querySelectorAll('[data-trip-status-for]').forEach(sel => {
        sel.addEventListener('change', () => updateTripStatus(sel.dataset.tripStatusFor, sel.value));
    });
}
...
function renderReviews() {
    document.querySelector('#reviewsTable tbody').innerHTML = AdminState.reviews.map(r => `
        <tr>
            <td>${r.users ? r.users.full_name : 'Unknown'}<br><small style="color:var(--gray-600);">${r.users ? r.users.email : ''}</small></td>
            <td>${'★'.repeat(r.rating)}${'☆'.repeat(5 - r.rating)}</td>
            <td>${r.comment || '<em style="color:var(--gray-600);">No comment</em>'}</td>
            <td><span class="badge ${r.is_approved ? 'badge-active' : 'badge-pending'}">${r.is_approved ? 'Approved' : 'Pending'}</span></td>
            <td class="table-actions">
                ${!r.is_approved ? `<button class="btn btn-success btn-sm" data-approve-review="${r.id}">Approve</button>` : `<button class="btn btn-outline btn-sm" style="color:var(--primary); border-color:var(--primary);" data-unapprove-review="${r.id}">Unpublish</button>`}
                <button class="btn btn-danger btn-sm" data-delete-review="${r.id}">Delete</button>
            </td>
        </tr>
    `).join('') || '<tr><td colspan="5" style="text-align:center; color:var(--gray-600);">No reviews submitted yet.</td></tr>';

    document.querySelectorAll('[data-approve-review]').forEach(btn => btn.addEventListener('click', () => setReviewApproval(btn.dataset.approveReview, true)));
    document.querySelectorAll('[data-unapprove-review]').forEach(btn => btn.addEventListener('click', () => setReviewApproval(btn.dataset.unapproveReview, false)));
    document.querySelectorAll('[data-delete-review]').forEach(btn => btn.addEventListener('click', () => deleteReview(btn.dataset.deleteReview)));
}

async function setReviewApproval(id, isApproved) {
    const { error } = await supabaseClient.from('reviews').update({ is_approved: isApproved }).eq('id', id);
    if (error) { showToast(error.message, 'error'); return; }
...
function setupForms() {
    ...
    document.getElementById('adForm').addEventListener('submit', async (e) => {
        ...
        try {
            let imageUrl = document.getElementById('adImage').value.trim() || null;
            ...
            if (file) {
                ...
                const { data: publicUrlData } = supabaseClient.storage.from('ads-images').getPublicUrl(filePath);
